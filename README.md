...
### 1) 管理规则
- 字段：
  - 域名：如 google.com（最大 128 字符，支持单条配置最多 3 个子域名，以英文逗号分割）
...
- 行为：读取当前窗口活动标签页的 URL → 解析主机名 → 提取 eTLD+1（处理常见多级公共后缀，如 co.uk、com.cn 等）→ 以追加（并应用英文逗号分割）的形式自动填入“域名”输入框，至多追加 3 个。
- 成功/异常：通过 Toast 提示结果；不可解析/权限不足会给出 warning/error，达到上限给 warning。

### 3) 导入/导出规则
- 切换表单：面板底部按钮可在“添加/编辑模式”与“导入/导出模式”之间切换
- 导入（文件）：
  - 仅接受 .json；文件 ≤ 1MB；条目 ≤ 200
  - 校验并规范化：domain 小写并去前导点，逗号分割；非法颜色统一降级为 'grey'
  - 合并策略：导入的规则若是和原有的配置中的单个域名产生重叠交叉，就会自动覆盖替换旧所在的整个对应规则（以导入项的为准）。不重复的项自动追加。
...
您可以自由地使用、修改和分发本项目代码，但请保留原作者版权声明。