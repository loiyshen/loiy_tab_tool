...
- 范围（本版本）：
  - 规则管理：添加、编辑、删除，支持 1 条规则配置多个子级域（上限 3 个），立即生效并同步存储。
  - 自动分组：标签页加载完成后触发；匹配首条命中的规则，创建或加入分组并设置名称与颜色。
  - 颜色策略：仅支持 Chrome 预设 tab group 颜色。
  - 侧边栏：可在约 320–600px 宽度内适配，无控件溢出或遮挡。
  - 规则数据导入/导出（新增多子域去重覆写算法）
  - 预设规则模板导入
  - 获取当前页面域名并追加填充到规则表单

3. 用户体验与交互流程
3.1 规则管理（Side Panel）
- 输入项：Domain（示例 google.com，支持包含多至 3 个逗号分隔的域名）、Group Name、Group Color（grey, blue, red, yellow, green, pink, purple, cyan）
- 约束：域名总长度最大 128 个字符，子域个数至多 3 个；分组名称最大 24 个字符（超出或重复冲突则阻止提交并提示）
...
- 导入行为（文件/粘贴）：
  ...
  - 校验与规范化：
    ...
    - 每个项需包含有效 domain（字符串，也可逗号分隔）、groupName（字符串）、color（预设颜色名之一）
    - 规范化 domain：切割逗号，trim 后 toLowerCase；去除前导点；color 校验非法则以 'grey' 兜底容错替换
  - 合并策略（无复杂选项界面，采用强去重原则）：
    - 导入项若与其内的某几个域名和现有历史配置产生了干涉或重叠，则将现有历史规则完全抛弃，以外部新导入的重叠规则集合为准代替老旧项覆盖。
    - 对于没有交集的域名进行直接追加。
  - 成功导入后：
...
- 功能与行为：
  ...
  - 如果输入栏为空或尚未抵达上限 3 个，自动将结果以追加合并（用英文逗号隔水）填充到“域名”输入框，并以 Toast 成功提示“已获取并填入域名：{domain}”。如果已经在此名单中则提醒“已在框中”。
- 域名提取规则：
...
- 触发与归位：
  - 表单级错误（如域名>128 加总字符、分隔多于 3 项或分组名>24）可同时在顶部显示多条错误消息，便于一次性提示所有问题
